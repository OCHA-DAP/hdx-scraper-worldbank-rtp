...
This script reads data from the World Bank Real Time Prices API for the food, energy, and currency exchange models. The data is split by country and one dataset is created per country with a resource for each model. A global dataset is also created with a resource for each model. The script runs every week.
 ...
 **hdx-scraper-worldbank_rtp** as specified in the parameter
 *user_agent_lookup*.
...
```shell
    pip install .
    python -m hdx.scraper.worldbank_rtp
```
